package com.springboot.cloud.redis.utils;
...
public class RedisUtils<T> {
    ...
    public boolean delete(String... key) {
        if (key != null && key.length > 0) {
            if (key.length == 1) {
                ...
                long i = redisTemplate.delete(CollectionUtils.arrayToList(key));
                if (i > 0) {
                    return true;
                } else {
                    return false;
    ...
    public Double incrementScore(String key, double score, String member) {
        try {
            return redisTemplate.opsForZSet().incrementScore(key, (T) member, score);
        } catch (Exception e) {
            e.printStackTrace();
            return 0.0;
        }
    ...
    public Long addToSet(String key, String value) {
        try {
            return redisTemplate.opsForSet().add(key, (T) value);
        } catch (Exception e) {
            e.printStackTrace();
            return 0L;
        }
     ...
     * @return
     */
    public int zSetScore(String key, String name) {
        try {
            Object score = redisTemplate.opsForZSet().score(key, name);
            return Double.valueOf(score.toString()).intValue();
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
